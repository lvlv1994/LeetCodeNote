...
```text
class Solution:    def lowestCommonAncestor(self, root: 'TreeNode', p: 'TreeNode', q: 'TreeNode') -> 'TreeNode':        self.result = None        self.helper(root, p, q)        return self.result        def helper(self, root, p, q):        if not root:            return False        left = self.helper(root.left, p, q)        right = self.helper(root.right, p, q)        mid = root == p or root == q                if mid + left + right >= 2:            self.result = root        return mid or left or right        
```
