...
```text
   public int binarySearch(List<Integer> list, int key){           int lo= 0;           int hi= list.size()-1;           while(lo<=hi){               int mid= lo+(hi- lo)/2;               if(list.get(mid)>=key){                   hi= mid-1;               }else{                   lo= mid+1;               }           }           return lo;       }
```
